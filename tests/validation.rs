...
use snipper::data::token_data::{
    ...
    get_and_save_erc20_by_token_address_no_validation, get_number_of_tokens, is_token_tradable,
};
use snipper::data::tokens::Erc20Token;
use snipper::events::PairCreatedEvent;
use snipper::swap::anvil_simlator::AnvilSimulator;
use snipper::swap::anvil_validation::TokenStatus;
use std::str::FromStr;
use std::sync::Arc;
...
async fn setup(token_address: Address) -> anyhow::Result<TestSetupValidation> {
    ...
    let token =
        get_and_save_erc20_by_token_address_no_validation(&pair_created_event, &client).await?;
...
async fn test_successful_token_validation() -> anyhow::Result<()> {
    ...
    let token_status = setup
        .anvil_simulator
        .validate_token_with_simulated_buy_sell(&setup.token)
        .await?;
